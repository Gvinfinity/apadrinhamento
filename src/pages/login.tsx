...
export const LoginPage = () => {
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            const status = await authCtx.login(data);

            navigate(status ? "/dashboard" : "/signup");
    ...
    return (
        <div className="w-full h-full flex flex-col items-center gap-5 p-2 pt-7 bg-zinc-800 overflow-y-scroll">
            ...
            <h1 className="text-4xl text-center font-extrabold text-cyan-200">Logue ou cadastre-se para começar</h1>
            <p className="mt-8 text-xl max-w-3xl text-rose-100 text-center">Use seu e-mail institucional para começar no no sistema de apadrinhamento</p>
            <form onSubmit={handleSubmit(onSubmit)} className="mt-8 w-5/6 lg:w-1/3 md:w-4/6 flex flex-col gap-3 bg-zinc-700 p-6 rounded-lg">
                ...
                <button className="mt-8 bg-amber-600 text-white text-xl font-bold py-2 px-4 rounded-lg cursor-pointer" type="submit">Começar</button>
            </form>
        </div>
    )
}